...
%DIF DEL Challenges_01_10_2022.tex   Tue Oct  4 10:14:11 2022
%DIF ADD Challenges_20_10_2022.tex   Thu Oct 20 13:56:51 2022
...
%DIF -------
	    \node at (14.75, 5.5) {Transition}; %DIF > 
%DIF -------
...
%DIF -------
	    \node at (6.5, -6.75) {Transition}; %DIF > 
%DIF -------
...
%DIF 323-324c323
%DIF < \jmlrheading{1}{2020}{1-48}{4/00}{10/00}{meila00a}{Théophile Champion and Marek Grze\'s and Howard Bowman}
%DIF < %TODO
%DIF -------
\jmlrheading{1}{2020}{1-48}{4/00}{10/00}{meila00a}{Théophile Champion and Marek Grze\'s and Lisa Bonheme and Howard Bowman} %DIF > 
%DIF -------
...
\begin{figure}[h]
	\begin{center}
	\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}]
%DIFDELCMD < 		\coordinate (A) at (13.5,8);
%DIFDELCMD < 		\coordinate (B) at (16,9);
%DIFDELCMD < 		\coordinate (C) at (16,11);
%DIFDELCMD < 		\coordinate (D) at (13.5,12);
%DIFDELCMD < 		\draw[fill=purple!20!white] (A) -- coordinate[pos=.6] (AB) (B)--(C)
%DIFDELCMD <          --coordinate[pos=.45] (CD) (D)
%DIFDELCMD <          --coordinate[pos=.55] (DA) cycle;
%DIFDELCMD < 	    \node at (14.75, 10) {$Policy$};
%DIFDELCMD < 

%DIFDELCMD < 		\draw[-latex] (11,6.5) -- (12.8,6.5) -- (12.8,10) -- (13.5,10);
%DIFDELCMD < 		\draw[-latex] (16, 10) -- (16.5, 10);
%DIFDELCMD < 		

%DIFDELCMD < 		\draw[fill=gray!20!white] (16.5,9) rectangle (17.5,11);
%DIFDELCMD < 	    \node at (17, 10) {$\hat{\pi}$};
%DIFDELCMD < 

%DIFDELCMD < 		\draw (16.6, 3.5) -- (16.6, 4) -- (17.75, 4) -- (17.75, 5.5);
...
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}]
...
\end{align}
where $\tilde{Q} = Q(o_\tau, s_\tau, \theta|\pi) = Q(\theta|\pi)Q(s_\tau|\theta,\pi)Q(o_\tau|s_\tau,\theta,\pi)$ and $\tilde{P}(o_\tau,s_\tau,\theta|\pi) = P(o_\tau|\pi)P(s_\tau|o_\tau,\pi)P(\theta|s_\tau,o_\tau,\pi)$. However, \DIFdelbegin \DIFdel{this is not the form of the expected free energy that is computed in the \mbox{%DIFAUXCMD
\citet{DeepAIwithMCMC} }\hspace{0pt}%DIFAUXCMD
approach. We will shortly present derivations that lead to the terms that are computed. Up to now, we left the definition of }\DIFdelend \DIFaddbegin \DIFadd{Equation \ref{eq:efe_fountas_defin} needs to be re-arranged to be computed in practice}\footnote{\DIFadd{By ``in practice", we mean ``in the code" or equivalently ``when implementing the approach".}}\DIFadd{, and Section \ref{ssection:efe_derivation_assumptions} will present this derivation. Finally, as shown in Figure \ref{fig:DAI_MC_agent}, }\DIFaddend the top-down attention parameter \DIFdelbegin \DIFdel{undefined. Formally, this parameter }\DIFdelend is computed as follows:
\begin{align*}
\omega_t = \frac{a}{1 + \exp(-\frac{b - D_t}{c})} + d,
\end{align*}
where $D_t = \kl{Q_{\phi_a}(a_t|s_t)}{P(a_t)}$, and $\{a, b, c, d\}$ are fixed hyperparameters. Intuitively, $\omega_t$ is high when the posterior over actions (from the policy network) is close to the prior over actions (from the expected free energy), and low when the posterior is far away from the prior. This, in turn, means that extra uncertainty is introduced into the transition mapping (see paragraph before Equation \ref{eq:initial_prior_actions}) when posterior over actions and prior over actions are very different. Finally, note that the number of terms required to compute the prior over actions (defined in \DIFaddbegin \DIFadd{Equation }\DIFaddend \ref{eq:initial_prior_actions}) grows exponentially with the time horizon of planning. Because this is intractable in practice, \citet{DeepAIwithMCMC} implemented a Monte-Carlo tree search (MCTS) algorithm to evaluate the expected free energy of each action (see below). Finally, action selection is performed by sampling from the following distribution:
\begin{align*}
...
\end{align}
By comparing \DIFaddbegin \DIFadd{the VFE in }\DIFaddend \eqref{eq:def_vfe_fountas} and \DIFaddbegin \DIFadd{the EFE in }\DIFaddend \eqref{eq:efe_fountas_defin} \DIFaddbegin \DIFadd{both from \mbox{%DIFAUXCMD
\citet{DeepAIwithMCMC}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , one can see an inconsistency. Namely, the parameters are seen as latent variables in the EFE definition, c.f., $\theta$ in \eqref{eq:efe_fountas_defin}, but they are regarded as parameters of neural netowrks in the VFE, c.f., $\theta_s$ and $\theta_o$ in \ref{eq:def_vfe_fountas}. This \DIFaddbegin \DIFadd{is important because $\theta$ cannot be a real and a random variable at the same, and if $\theta$ is a random variable one must define its probability density, i.e., $P(\theta)$. Additionally, this inconsistency }\DIFaddend raises the question of whether the EFE is really the expectation of the VFE. To sum up, the $DAI_{MC}$ agent is equipped with four deep neural networks modelling $Q_{\phi_a}(a_t|s_t)$, $Q_{\phi_s}(s_t)$, $P_{\theta_s}(s_t|s_{t-1},a_{t-1})$, and $P_{\theta_o}(o_t|s_t)$. The weights of those networks are optimised using back-propagation to minimise the VFE given by \eqref{eq:def_vfe_fountas}. Note, \eqref{eq:def_vfe_fountas} decomposes into two KL-divergence terms that can be computed analytically, and the expectations can be approximated using a Monte-Carlo estimate. Also, because $P_{\theta_o}(o_t|s_t)$ is modelled as a product of Bernoulli distributions, the logarithm of $P_{\theta_o}(o_t|s_t)$ reduces to the binary cross entropy.

\subsubsection{Independence assumptions and the expected free energy} \DIFaddbegin \label{ssection:efe_derivation_assumptions}
\DIFaddend 

The EFE as stated in \DIFaddbegin \DIFadd{Equation }\DIFaddend \eqref{eq:efe_fountas_defin} needs to be re-arranged because it cannot be easily evaluated. \DIFdelbegin \DIFdel{First, using }\DIFdelend \DIFaddbegin \DIFadd{We therefore present the derivation proposed by \mbox{%DIFAUXCMD
\citet{DeepAIwithMCMC}}\hspace{0pt}%DIFAUXCMD
. Then, we highlight two independence assumptions, i.e., $s_\tau \indep \theta \,\,|\,\, \pi$ and $s_\tau \indep \theta \,\,| \,\,\pi, o_\tau$, used without proof. Finally, we propose an alternative derivation that does not require those two assumptions and produces a simpler result. Using }\DIFaddend the product rule of probability, one can see that $Q(s_\tau, \theta|\pi) = Q(\theta|s_\tau,\pi)Q(s_\tau|\pi)$ and $\tilde{P}(o_\tau,s_\tau,\theta|\pi) = P(o_\tau|\pi)P(s_\tau|o_\tau,\pi)P(\theta|s_\tau,o_\tau,\pi)$. Using those two factorisations, the EFE given in \eqref{eq:efe_fountas_defin}, i.e.,
\begin{align*}
...
\DIFdelbegin \DIFdel{In the supplementary material of \mbox{%DIFAUXCMD
\citep{DeepAIwithMCMC}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{First}\DIFaddend , the second term of Equation \eqref{eq:efe_rearrangedd} is \DIFdelbegin \DIFdel{re-aranged }\DIFdelend \DIFaddbegin \DIFadd{re-arranged into entropy terms for which an analytical solution exists. In the supplementary material of \mbox{%DIFAUXCMD
\citep{DeepAIwithMCMC}}\hspace{0pt}%DIFAUXCMD
, the derivation goes }\DIFaddend as follows:
\begin{align}
...
\end{align*}
where in the first line a distribution was renamed, i.e., $\tilde{P}(s_\tau|o_\tau, \pi) \delequal Q(s_\tau|o_\tau, \pi)$, two different factorizations of $\tilde{Q}$ are used going from \DIFdelbegin \DIFdel{first to }\DIFdelend \DIFaddbegin \DIFadd{the first to the }\DIFaddend second line, the linearity of expectations was used between the first and second line, and the expectation w.r.t $\theta$ was dropped (between \DIFdelbegin \DIFdel{line }\DIFdelend \DIFaddbegin \DIFadd{lines }\DIFaddend two and three) because the expectation of a constant is the constant itself. Importantly, the above derivation does not make any assumption of independence, and leads to a simpler result. \DIFaddbegin \DIFadd{This alternative derivation is beneficial as it produces a stronger derivation because it makes less (unjustified) assumptions. The simpler result produced by this derivation, also have a practical implication. Indeed, the expectation w.r.t. $Q(\theta|\pi)$ disapears and the expectation w.r.t. $Q(o_\tau|\theta,\pi)$ is now w.r.t. $Q(o_\tau|\pi)$. Those two changes sugguest that a different implementation of this term is required.
}\DIFaddend 

\subsubsubsection{Re-arranging the third term of Equation \eqref{eq:efe_rearrangedd} from \citet{DeepAIwithMCMC}}

\DIFdelbegin \DIFdel{We now, }\DIFdelend \DIFaddbegin \DIFadd{For completeness, we now }\DIFaddend focus on the third term of \eqref{eq:efe_rearrangedd}, which can be re-arranged as follows:
\begin{align*}
...
\end{align*}
\DIFaddbegin \DIFadd{To sum up, this derivation provides an expression based on the following two entropy terms: $H[ Q(o_\tau|s_\tau, \pi) ]$ and $H[Q(o_\tau|s_\tau,\theta,\pi)]$, which the authors claim can be estimated (c.f. Apprndix B for more details). Note that our proposed alternative to the EFE (see below) uses this derivation for the third term of Equation }\eqref{eq:efe_rearrangedd}\DIFadd{.
}\DIFaddend 
...
\end{align}
\DIFdelbegin \DIFdel{The way this expression is evaluated in practice is described }\DIFdelend \DIFaddbegin 

\noindent \DIFadd{Note, in Section \ref{ssection:fountas_paper}, we focused on presenting the approach of \mbox{%DIFAUXCMD
\citet{DeepAIwithMCMC} }\hspace{0pt}%DIFAUXCMD
--- as presented in their paper --- in the most consistent manner posible. More details about the implementation of Equation \ref{eq:efe_rearranged_fountas} is presented }\DIFaddend in Appendix B, along with some discrepancies between the paper and the code.
...
\DIFdelbegin \DIFdel{Despite what we have seen so far, and the discrepancies described }\DIFdelend \DIFaddbegin \noindent \DIFadd{As explained in Section \ref{ssection:efe_derivation_assumptions}, it is not clear how to prove the two independence assumptions (i.e., $s_\tau \indep \theta \,\,|\,\, \pi$ and $s_\tau \indep \theta \,\,| \,\,\pi, o_\tau$) used by \mbox{%DIFAUXCMD
\citet{DeepAIwithMCMC} }\hspace{0pt}%DIFAUXCMD
in their derivation of the expected free energy. Additionally, as presented }\DIFaddend in Appendix B, \DIFaddbegin \DIFadd{we were able to identify discrepancies between the paper and the code. Despite those two points, }\DIFaddend $DAI_{MC}$ does solve the dSprites environment.
...
\end{align*}
which minimises the variational free energy. Finally, the critic is trained by minimising the MSE between the target EFE as defined in \eqref{eq:efe_beren} and the ouput of the critic \DIFaddbegin \DIFadd{$\mathcal{G}_{\theta_a}(o_\tau, \bigcdot\,)$}\DIFaddend :
\begin{align*}
...
\begin{figure}[H]
	\begin{center}
	\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}]
%DIFDELCMD < 		\coordinate (A) at (13.5,12.5);
%DIFDELCMD < 		\coordinate (B) at (16,13.5);
%DIFDELCMD < 		\coordinate (C) at (16,15.5);
%DIFDELCMD < 		\coordinate (D) at (13.5,16.5);
%DIFDELCMD < 		\draw[fill=purple!20!white] (A) -- coordinate[pos=.6] (AB) (B)--(C)
%DIFDELCMD <          --coordinate[pos=.45] (CD) (D)
%DIFDELCMD <          --coordinate[pos=.55] (DA) cycle;
%DIFDELCMD < 	    \node at (14.75, 14.5) {$Policy$};
%DIFDELCMD < 		\draw[-latex] (12.8,6.5) -- (12.8,14.5) -- (13.5,14.5);
%DIFDELCMD < 		\draw[-latex] (16, 14.5) -- (16.5, 14.5);
%DIFDELCMD < 		\draw[fill=gray!20!white] (16.5,13.5) rectangle (17.5,15.5);
...
%DIFDELCMD < 		\coordinate (C) at (16,11);
%DIFDELCMD < 		\coordinate (D) at (13.5,12);
%DIFDELCMD < 		\draw[fill=yellow!20!white] (A) -- coordinate[pos=.6] (AB) (B)--(C)
%DIFDELCMD <          --coordinate[pos=.45] (CD) (D)
%DIFDELCMD <          --coordinate[pos=.55] (DA) cycle;
%DIFDELCMD < 	    \node at (14.75, 10) {$Critic$};
%DIFDELCMD < 

%DIFDELCMD < 		\draw[-latex] (12.8,6.5) -- (12.8,10) -- (13.5,10);
%DIFDELCMD < 		\draw[-latex] (16, 10) -- (16.5, 10);
%DIFDELCMD < 		
...
%DIFDELCMD < 		\draw[fill=gray!20!white] (11,9) rectangle (12,11);
%DIFDELCMD < 	    \node at (11.5, 10) {$o_t$};
%DIFDELCMD < 

%DIFDELCMD < 		\draw[fill=blue!20!white] (11,3.5) rectangle (12,5.5);
%DIFDELCMD < 	    \node at (11.5, 4.5) {$a_t$};
%DIFDELCMD < 

%DIFDELCMD < 		\coordinate (A) at (13.5,3.5);
%DIFDELCMD < 		\coordinate (B) at (16,4.5);
%DIFDELCMD < 		\coordinate (C) at (16,6.5);
%DIFDELCMD < 		\coordinate (D) at (13.5,7.5);
%DIFDELCMD < 		\draw[fill=orange!20!white] (A) -- coordinate[pos=.6] (AB) (B)--(C)
%DIFDELCMD <          --coordinate[pos=.45] (CD) (D)
%DIFDELCMD <          --coordinate[pos=.55] (DA) cycle;
%DIFDELCMD < 	    \node at (14.75, 5.5) {$Transition$};
%DIFDELCMD < 

%DIFDELCMD < 		\draw[fill=gray!20!white] (16.5,4.5) rectangle (17.5,6.5);
%DIFDELCMD < 	    \node at (17, 5.5) {$\mathring{o}_{t+1}$};
%DIFDELCMD < 

%DIFDELCMD < 		\draw[-latex] (12,4.5) -- (13.5,4.5);
%DIFDELCMD < 		\draw[-latex] (12,10) -- (13.5,10);
%DIFDELCMD < 		\draw[-latex] (12.8,6.5) -- (13.5,6.5);
%DIFDELCMD < 

%DIFDELCMD < 		\draw[-latex] (16,5.5) -- (16.5,5.5);
%DIFDELCMD < 
...
While CKA leads to accurate results in practice, it can be overly sensitive to differences in neural architectures \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citet{Maheswaranathan2019}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Maheswaranathan2019}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , and can thus underestimate the similarity between activations coming from layers of different type (e.g., convolutional and deconvolutional). Thus, we will only discuss the variation of similarity when analysing such cases.
...
\end{align}
The VFE \DIFaddbegin \DIFadd{of Equation \ref{eq:vfe_defi} }\DIFaddend can be computed in a similar way to the VAE agent. Put simply, this agent takes random actions, and stores its experiences in a \DIFdelbegin \DIFdel{queue }\DIFdelend \DIFaddbegin \DIFadd{replay buffer }\DIFaddend (c.f. Section \ref{ssec:env_agent_iter}). Then, batches of experiences ($o_{t-1}$, $a_{t-1}$, $o_t$, $r_t$, $done$) are sampled from the \DIFdelbegin \DIFdel{queue}\DIFdelend \DIFaddbegin \DIFadd{replay buffer}\DIFaddend . The observations at time step $t - 1$ are feed into the encoder, which outputs the mean and log variance of a Gaussian distribution $Q_{\phi_s}(s_{t-1}) = \mathcal{N}(s_{t-1};\mu, \sigma)$. A latent state is sampled from $Q_{\phi_s}(s_{t-1})$ using the re-parameterisation trick, and is then provided as input to the transition network along with action $a_{t-1}$. The transition network outputs the parameters of the Gaussian distributions $P_{\theta_s}(s_t|s_{t-1},a_{t-1}) = \mathcal{N}(s_t;\mathring{\mu}, \mathring{\sigma})$. Additionally, the observations at time step $t$ can be fed into the encoder to get the parameters of $Q_{\phi_s}(s_t) = \mathcal{N}(s_t;\hat{\mu}, \hat{\sigma})$. Sampling from $Q_{\phi_s}(s_t)$ using the reparameterisation trick gives a state $\hat{s}_t$ that when given as input to the decoder produces the parameters of a product of Bernoulli distributions $\hat{o}_{t+1}$. The KL-divergence between $Q_{\phi_s}(s_t)$ and $P_{\theta_s}(s_t|s_{t-1},a_{t-1})$ is computed analytically, and the logarithm of $P_{\theta_o}(o_t|s_t)$ reduces to the \DIFdelbegin \DIFdel{BCE }\DIFdelend \DIFaddbegin \DIFadd{binary cross entropy (BCE) }\DIFaddend because $P_{\theta_o}(o_t|s_t)$ is a product of Bernoulli distributions. Next, the VFE is obtained by subtracting the BCE from the KL-divergence, and back-propagation is used to update the weights of the encoder, decoder and transition networks. Figure \ref{fig:HMM} illustrates the HMM agent. 
...
where $Q_{\phi_s}(s_{t+1})$ can be computed by feeding the image $o_{t+1}$ sampled from the \DIFdelbegin \DIFdel{queue }\DIFdelend \DIFaddbegin \DIFadd{replay buffer }\DIFaddend as input to the encoder, $G_{t+1}(a_t)$ is the expected free energy at time $t+1$ after taking action $a_t$ (see below), and $\gamma$ is a discount factor. \DIFaddbegin \DIFadd{Note, the above Equation is an application on Bellman's equation \mbox{%DIFAUXCMD
\citep{Bellman1952} }\hspace{0pt}%DIFAUXCMD
to the expected free energy. }\DIFaddend Also, as for the DQN agent, we improved the training stability by implementing a target network $\hat{\mathcal{G}}_{\hat{\theta}_a}$, which is structurally identical to the critic and whose weights are synchronised with the weights of the critic every $K$ (learning) iterations. The last question to answer before focusing on the subject of the EFE is: how does the CHMM select the action to be performed in the environment?
...
\end{align}
where $P(o_\tau|s_\tau)$ is the likelihood mapping, $Q(s_\tau | \pi)$ is the variational distribution, and \DIFdelbegin \DIFdel{literally }\DIFdelend \DIFaddbegin \DIFadd{in the literature }\DIFaddend $P(o_\tau, s_\tau)$ is \DIFaddbegin \DIFadd{called }\DIFaddend the generative model but is better understood as a target distribution encoding the prior preferences of the agent. Indeed, assuming the standard generative model of active inference (i.e., a partially observable Markov decision process), the hidden states $s_\tau$ should depend on $s_{\tau-1}$ and $a_{\tau-1}$. \DIFdelbegin \DIFdel{This point is important because it impacts the question of whether $P(o_\tau, s_\tau)$ is indeed the generative model, and therefore whether the expected free energy is the expectation of the variational free energy. }\DIFdelend \DIFaddbegin \textbf{\DIFadd{This point is important because it impacts the question of whether $P(o_\tau, s_\tau)$ is indeed the generative model, and therefore whether --- as the name sugguest --- the expected free energy is the expectation of the variational free energy. More importantly, according to the free energy principle, an active inference agent is supposed to minimise its (variational) free energy. However, if no relationship can be established between the expected and variational free energy, then one cannot claim that an agent minimising expected free energy also minimises its variational free energy, i.e., one cannot claim that an active inference agent behave according to the free energy principle.}} \DIFaddend Additionally, we need to re-arrange the definition of the EFE stated in \eqref{eq:efe_definition} to allow rewards to be incorporated:
\begin{align}
...
\end{align}
where we used \DIFaddbegin \DIFadd{the fact }\DIFaddend that the summation over all $o_\tau$ is a normalisation term, i.e., a constant. Using \eqref{eq:prior_pref}, we can now create an estimate of the extrinsic value as follows:
\begin{align*}
...
\end{align}
where $\hat{s}_t \sim Q_{\phi_s}(s_t)$, $P_{\theta_s}(s_{t+1}|s_t, a_t)$ is known from the generative model, $Q_{\phi_s}(s_{t+1})$ is known from the variational distribution, the KL-divergence can be estimated using an analytical solution, $\psi$ is a hyperparameter modulating the precision of the prior preferences, and $r_{t+1}$ is the reward obtained at time step $t+1$. \DIFaddbegin \DIFadd{As shown in Figure \ref{fig:reward_usage}, the reward at time step $t+1$ is used to compute the target values that must be predicted by the critic network.
}\DIFaddend 
...
%DIFDELCMD <          --coordinate[pos=.55] (DA) cycle;
%DIFDELCMD < 	    \node at (14.75, 14.5) {$Policy$};
%DIFDELCMD < 		\draw[-latex] (11,6.5) -- (12.8,6.5) -- (12.8,14.5) -- (13.5,14.5);
%DIFDELCMD < 		\draw[-latex] (16, 14.5) -- (16.5, 14.5);
%DIFDELCMD < 		\draw[fill=gray!20!white] (16.5,13.5) rectangle (17.5,15.5);
%DIFDELCMD < 	    \node at (17, 14.5) {$\hat{\pi}$};
%DIFDELCMD < 
...
\begin{figure}[H]
	\begin{center}
	\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=0.75]
%DIFDELCMD < 	    \node at (0, 0) {\includegraphics[scale=0.5]{vae_reconstruction.png}};
%DIFDELCMD < 	    \node[gray!60!black] at (9, 0) {Ground truth:};
%DIFDELCMD < 	    \node[gray!60!black] at (9, 0.5) {Reconstruction:};
%DIFDELCMD < 		\draw[fill=Green] (11, 0.2) rectangle (12, -0.2);
%DIFDELCMD < 		\draw[fill=Red] (11, 0.7) rectangle (12, 0.3);
%DIFDELCMD < 	\end{tikzpicture}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=0.75]
	    \node at (0, 0) {\includegraphics[scale=0.5]{vae_reconstruction.png}};
...
%DIFDELCMD < 	    \node[gray!60!black] at (9, 0.5) {Reconstruction:};
%DIFDELCMD < 		\draw[fill=Green] (11, 0.2) rectangle (12, -0.2);
%DIFDELCMD < 		\draw[fill=Red] (11, 0.3) rectangle (12, 0.7);
%DIFDELCMD < 	\end{tikzpicture}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=0.75]
	    \node at (0, 0) {\includegraphics[scale=0.3]{hmm_reconstruction.png}};
...
\begin{figure}[H]
	\begin{center}
	\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=0.75]
%DIFDELCMD < 	    \node at (0, 0) {\includegraphics[scale=0.3]{chmm_reconstruction}};
%DIFDELCMD < 	    \node[gray!60!black] at (6, 0) {Ground truth:};
%DIFDELCMD < 	    \node[gray!60!black] at (6, 0.5) {Reconstruction:};
%DIFDELCMD < 		\draw[fill=Green] (8, 0.2) rectangle (9, -0.2);
%DIFDELCMD < 		\draw[fill=Red] (8, 0.3) rectangle (9, 0.7);
%DIFDELCMD < 	\end{tikzpicture}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=0.75]
...
In contrast, the CHMM maximising reward, keeps on selecting the actions right and left, which enables it to drag the shape towards the appropriate corner (see Figure \ref{fig:action_picked_reward}). Also, as shown in Figure \ref{fig:entropy_action_reward}, the entropy of the prior over actions remained a lot higher than zero. \DIFdelbegin %DIFDELCMD < 
...
\DIFaddbegin \begin{figure}[H]
    ...
    \caption{\DIFaddFL{(a) shows the action taken for each planning iteration when the CHMM is minimising expected free energy.
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \begin{subfigure}{.4\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[draft=false,width=\linewidth]{ActionPicked_efe_3}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \label{fig:action_picked_efe}
%DIFDELCMD <     \end{subfigure}%%%
%DIF < 
    %DIFDELCMD < \begin{subfigure}{.4\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[draft=false,width=\linewidth]{EntropyPriorActions_efe_3}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \label{fig:entropy_action_efe}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD <     \begin{subfigure}{.4\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[draft=false,width=\linewidth]{ActionPicked_reward}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \label{fig:action_picked_reward}
%DIFDELCMD <     \end{subfigure}%%%
%DIF < 
    %DIFDELCMD < \begin{subfigure}{.4\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[draft=false,width=\linewidth]{EntropyPriorActions_reward}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \label{fig:entropy_action_reward}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD < 
...
\DIFdelFL{(a) shows the action taken for each planning iteration when the CHMM is minimising expected free energy.
    ...
    %DIFAUXCMD
%DIFDELCMD < \label{fig:actions_from_critic}
...
%DIFDELCMD < \definecolor{LightBlueCHMM}{RGB}{51, 187, 238}
%DIFDELCMD < \definecolor{GreenCHMM}{RGB}{0, 153, 136}
%DIFDELCMD < \definecolor{GrayCHMM}{RGB}{187,187,187}
%DIFDELCMD < \definecolor{OrangeCHMM}{RGB}{255, 112, 67}
%DIFDELCMD < \definecolor{PinkCHMM}{RGB}{238, 51, 119}
%DIFDELCMD < \definecolor{DarkBlueCHMM}{RGB}{0, 119, 187}
%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < 	\begin{center}
%DIFDELCMD < 	\begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=1]
%DIFDELCMD < 	    \node at (-2, 0) {\includegraphics[draft=false,width=0.5\linewidth]{IG_progressive}};
%DIFDELCMD < 

%DIFDELCMD < 	    \node[black] at (4.675, -1) {CHMM[0\%]:};
%DIFDELCMD < 		\draw[fill=DarkBlueCHMM] (6, -0.8) rectangle (7, -1.2);
%DIFDELCMD < 

%DIFDELCMD < 	    \node[black] at (4.675, -0.5) {CHMM[1\%]:};
%DIFDELCMD < 		\draw[fill=GrayCHMM] (6, -0.7) rectangle (7, -0.3);
%DIFDELCMD < 

%DIFDELCMD < 	    \node[black] at (4.675, 0) {CHMM[5\%]:};
%DIFDELCMD < 		\draw[fill=OrangeCHMM] (6, -0.2) rectangle (7, 0.2);
%DIFDELCMD < 

%DIFDELCMD < 	    \node[black] at (4.585, 0.5) {CHMM[15\%]:};
%DIFDELCMD < 		\draw[fill=GreenCHMM] (6, 0.3) rectangle (7, 0.7);
%DIFDELCMD < 

%DIFDELCMD < 	    \node[black] at (4.585, 1) {CHMM[25\%]:};
%DIFDELCMD < 		\draw[fill=PinkCHMM] (6, 0.8) rectangle (7, 1.2);
%DIFDELCMD < 

%DIFDELCMD < 	    \node[black] at (4.585, 1.5) {CHMM[50\%]:};
%DIFDELCMD < 		\draw[fill=LightBlueCHMM] (6, 1.3) rectangle (7, 1.7);
%DIFDELCMD < 
...
\DIFdelFL{This figure illustrates the total reward aggregated by CHMM agents during the 500K iterations of training. All the agents start by only maximising reward, and after 200K training iterations, X\% of the information gain is added to the objective function. Note, even adding 1\% of the information gain is enough to drastically reduce the total reward aggregated by the agent. The differences in trajectories before 200K are arbitrary, arising from differences in random initializations.}}
   %DIFAUXCMD
%DIFDELCMD < \label{fig:IG_progressive}
%DIFDELCMD < \end{figure}
%DIFDELCMD < }
%DIFDELCMD < 
...
\begin{figure}[H]
	\begin{center}
	\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=0.75]
%DIFDELCMD < 	    \node at (0, 0) {\includegraphics[scale=0.22]{DAI_reconstruction.png}};
%DIFDELCMD < 	    \node[gray!60!black] at (8, 0) {Ground truth:};
%DIFDELCMD < 	    \node[gray!60!black] at (8, 0.5) {Reconstruction:};
%DIFDELCMD < 		\draw[fill=Green] (10, 0.2) rectangle (11, -0.2);
%DIFDELCMD < 		\draw[fill=Red] (10, 0.3) rectangle (11, 0.7);
%DIFDELCMD < 	\end{tikzpicture}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[square/.style={regular polygon,regular polygon sides=4}, scale=0.75]
...
Overall, those approaches brought interesting ideas such as: using deep neural networks to predict the parameters of the distributions of interest, using Monte-Carlo tree search for planning in active inference, and using a bootstrap estimate of the expected free energy to train a critic network. Yet, we struggled to replicate some of the results claimed\DIFaddbegin \DIFadd{, e.g. training $DAI_{MC}$ on the animal AI environment, and getting access to the code of \mbox{%DIFAUXCMD
\citet{schneider2022active} }\hspace{0pt}%DIFAUXCMD
was not possible}\DIFaddend . Ideally, future research should draw inspiration from the open science framework, by making the code that produced the claimed results open source. Also, the definition of the expected free energy varied between papers. This suggests that additional research is required to clarify the definition and justification of the expected free energy both in tabular and deep active inference. To sum up, recent research on deep active inference \citep{DeepAIwithMCMC,DeepAI,rood2020deep,sancaktar2020endtoend,DAI_Kai,DAI_POMDP,ccatal2020learning} has made an important first step towards a complete deep active inference agent, but a number of details still need to be honed\DIFaddbegin \DIFadd{, e.g., the definition and derivation of the expected free energy, the correctness of the implemention, and reproducibilty of research. For more details, the reader is referred to the subsections of Section \ref{sec:existing_research} corresponding to the approaches just mentioned}\DIFaddend .
...
To conclude, the field of deep active inference has benefited from a large variety of ideas from the reinforcement and deep learning literature. In the future, it would be valuable to provide an approach that satisfies the following five desirata: (i) the approach is complete, i.e., it is composed of an encoder, a decoder, a transition network, a policy network and (optionally) a critic network, (ii) the mathematics underlying the approach is \DIFdelbegin \DIFdel{principled and errorless }\DIFdelend \DIFaddbegin \DIFadd{errorless and consistent with the free energy principle}\DIFaddend , (iii) the implementation \DIFdelbegin \DIFdel{is correct, and }\DIFdelend consistent with the mathematics, (iv) the code is publicly available so that the correctness of the implementation can be verified and the results reproduced, and (v) the approach is able to solve tasks with a large input space, e.g. image-based tasks. We believe that such an approach will benefit the field of deep active inference by providing a strong and reproducible baseline against which future research could benchmark.
